process METAPROVIZ_POOLESTIMATION {
    tag "$meta.id"
    ...
    // the sample id attached to every file, not just se.
    tuple val(meta), path("*.pool_filtered.rds")      , emit: se
    ...
    def VERSION = '1.0.0'
    """
    touch ${prefix}.pool_filtered.rds
    touch ${prefix}.cv.tsv ${prefix}.high_var.txt \\
          ${prefix}.plots.rds ${prefix}.report.html ${prefix}.log
